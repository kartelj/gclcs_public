	...
	bool operator < (const QNode &other) const
	{
		if (ub == other.ub) {
			if (node->l_v == other.node->l_v) // breaking ties I
			    return  accumulate(node->QL.begin(), node->QL.end(), 0) < accumulate(other.node->QL.begin(), other.node->QL.end(), 0); // node->QL[ 0 ] < other.node->QL[ 0 ] ;
...
private:
	...
	void printNode(const shared_ptr<Node> &v, bool withParent = true);
        bool satisfied(const shared_ptr<Node> &v, int i_0);
