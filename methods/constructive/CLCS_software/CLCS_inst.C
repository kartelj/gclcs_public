...
CLCS_inst::CLCS_inst(const std::string &fname) : filename(fname)
{
	...
	n = 0;
        //RYMKSWHDBVN
        //ACAGGAAGGGA
        std::map<char, char> transform; cout << fname << endl;
        string fname1 = fname.substr(fname.find_last_of("\/") + 1);
        size_t dot_i = fname1.find_last_of('.');
        std::string fsyst = fname1.substr(0, dot_i);  
        if( bacteria() == 1)   //fsyst.substr(0, 1)  ==" B") // real-world instances
        {
          cout << "Bacteriae -------------------------------------> " << endl;
          transform['r'] = 'a';
          transform['y'] = 'c';       
	...
	S = std::vector<std::vector<int>>(m, std::vector<int>());
		...
		for (int j = 0; j < len; ++j) { // read characters...
			int a; char sx = seq[j];
                        ...
                        if(transform.find(sx) != transform.end())
                            sx = transform[ sx ];

			if (char2Int.count(sx) == 0) {

				char2Int[ sx ] = count;
				int2Char.push_back( sx );
				a = count;
				count++;
			} else
				a = char2Int[ sx ];
	...
	{   string sq;
		...
		int a;
		if (char2Int.count(sq[j]) == 0) {

		    char2Int[sq[j]] = count;
		    int2Char.push_back(sq[j]);
		    a = count;
		    count++;
		} else
		    a = char2Int[sq[j]];
		P[ ix ][ j ] = a; //cout << "a: " << a << endl; // store into constrained string P_i
	   }
        }
	// up to the same length:
	for(int i = 0; i < S.size(); ++i) 
	{   
            vector<int>s_ii(n, sigma); 
...
int CLCS_inst::ub_2(int i1, int j1)
{
...
void CLCS_inst::similarity()
{
	...
	{
             int sim = 0; // cout << "i: " << i << " j: " << j << endl; 
	     //sim = ub_2(i, j);
             for(int i1 = 0; i1 < S[i].size(); ++i1)
             {
                 if(S[i][i1] == S[j][i1] and S[i][i1] != sigma and S[j][i1] != sigma )
	           sim++;
	     }
	     // cout << max_sim << "  " << sim << endl;
	     if(max_sim > sim) 
	     {
		max_sim = sim;
		c_x = i; 
...
void CLCS_inst::store_M() //<< creating M structure...
{
	...
	for (int i = 0; i < m - 1; i++) 
        ...
        similarity(); cout << "c_x: " << c_x << " c_y: " << c_y << " c_p: " << c_p << endl;
        //M.push_back(clcs_m_ij(c_x, c_y, c_p));	 
}
...
std::vector<std::vector<int>>  CLCS_inst::embed(int p_index)
{       
        // preporcess Embed data structure
	std::vector<std::vector<int>> embed_end_pi = std::vector<std::vector<int>>(m, std::vector<int>());
	for (int i = 0; i < m; i++) {

		int p_u = P[ p_index ].size() - 1;
		std::vector<int> s_i = S[i];

		for (unsigned int j = s_i.size() - 1; j >= 0 && p_u >= 0; j--) {
			if (s_i[j] == P[ p_index ][p_u]) {
				
                                embed_end_pi[i].push_back(j);
				p_u--;
			}
		}

		std::reverse(embed_end_pi[i].begin(), embed_end_pi[ i ].end());
		embed_end_pi[ i ].push_back(s_i.size()); // border case for once the constraint is fulfilled
        }
        return embed_end_pi; 
}

void CLCS_inst::embeding()
{
     /**Embed[p, i, u ] **/
...
{       
       // preporcess Embed data structure ==> for (m,k)--CLCS problem (update need): 3D ==> tomorrow
