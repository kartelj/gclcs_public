...
void BeamSearch::startSearch()
{
	...
	{       cout << "#interation " << iter << endl;
                //beam[0]->print(); // cout << "h: " << beam[0]->h << endl;   
		//beam.reserve(500000);   
		...
		if (filtering() == 1)
		    filter(extensions);
		beam.clear(); //beam.reserve(500000);
        ...
        cout << "S_best: " << endl; sol->s.clear(); 
        s_best->print(); getSolution(); //cout << "|s| = " << sol->s.size() << endl;
...
vector<NodeExtension> BeamSearch::extendAndEvaluate(const vector<shared_ptr<Node>>& beam)
{
	...
	for (NodeExtension &extension : extensions) {
	     if (guidance() > 0) {
		 extension.h = h(*extension.node, inst, px); //extension.node.get()->print(); cout << "h =  " << extension.h << endl;
		 if (h2 != nullptr) {
		     extension.h2 = h2(*extension.node, inst, px); //cout << "h2= " << extension.h2 << endl;
                     if(turn_cutoff() == 1) 
                       extension.cut_off = evaluate_cutoff(*extension.node);
		 }
             } else{  // use UB as a guidance:
		    extension.h = ub(*extension.node, inst); //cout << "UB: " << extension.h << endl; //extension.node.get()->print();
		    if( h2 != nullptr) 
			extension.h2 = h2(*extension.node, inst, px); //cout << extension.h << " " << extension.h2 << endl; 
                    if(turn_cutoff() == 1) {
                       extension.cut_off = evaluate_cutoff(*extension.node);
                      // cout << "extension.cut_off val: " << extension.cut_off << endl;
                    }
               }
	}
	std::sort(extensions.begin(), extensions.end(), std::greater<NodeExtension>());
        ...
        {
	extensions[0].node->print(); 
	cout << "h-------------> " << extensions[0].h << " " << extensions[0].h2 <<  endl;
	}
	return extensions;
}
...
int BeamSearch::determine_p( vector<NodeExtension> &extensions )
{
	...
	for (NodeExtension &ext : extensions) {
		...
		for (int i = 0; i < inst->m; ++i) {
			if (l_mx > (int) inst->S[i].size() - v.pL[i] + 1 - l_m)
		...
		bool skipHeuristic = false;int charNum = -1;
		    ...
		    for (auto &it : sigma) 
				...
				double g = clcs_eval::greedy(*child, inst);   
                                //cout << "g: " << g <<  " " << letter << endl;
                                /*bool f = branchOrNot(*child);
                                if(f) 
                                {  cout << "--------------->" <<  branched << endl; branched++; }*/
				if (g <= bestVal ) { // and feasNode(child)
				    bestVal = g;
				    bestChild = child;
		    ...
		    v = bestChild; update_embed_struct(*v);    //temp_best); 
                ...
                if(sigma.size() > 0) 
                {  cout << "temp_best = " << temp_best.size() << endl;
