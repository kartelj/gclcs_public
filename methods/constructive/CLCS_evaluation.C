...
double greedy(const Node &v, const CLCS_inst *inst)
{
	...
	for (int i = 0; i < inst->m; i++) {
	     g += (v.pL[i] - parent->pL[i] + 1) / (double) (inst->S[i].size() - parent->pL[i] + 1);
             if( l_min > (v.pL[i] - parent->pL[i] ) )
                 l_min = v.pL[i] - parent->pL[ i ];
	}   
         int q = 0;
        /* for(int i = 0 ; i < inst->k; ++i) 
         {
             
              if (parent->QL[ i ] < inst->P[ i ].size()){
                  q = (v.QL[ i ] != parent->QL[ i ]) ? 1 : 0; // strong match	
              }
...
{
	long double h_val = 1.0;
	for (int i = 0; i < inst->m; ++i) {
		h_val *= inst->P_m[ p ][inst->S[i].size() - v.pL[i] + 1];
	}
	// check this?
...
int ub(const Node &v, const CLCS_inst *inst)
{
	...
	if(inst->M_lcs.size() > 0)
	   ub_2 = clcs_eval::ub2(v, inst);
        //int ub_3 = inst->M[ 0 ][ v.pL[ 0 ] - 1 ][ v.pL[ 1 ] - 1 ][v.QL[ 0 ] ];
	...
	for(int i = 0; i < v.QL.size(); ++i) 
        ...
        if(inst->M_lcs.size() > 0 )
	   return std::min(ub_1, ub_2); //+ penality; //std::min(ub_2, ub_3));
        else
           return ub_1;
}
